(
  function initAlephbetGame(globalScope) {
    ...
    function createCard(letterData, statusElement, babyModeState) {
    ...
    function buildCards(trackElement, statusElement, babyModeState) {
      ALEPHBET.forEach((letterData) => {
        const card = createCard(letterData, statusElement, babyModeState);
    ...
    function setupBabyMode(trackElement, statusElement) {
      ...
      toggleElement.addEventListener("change", () => {
        ...
        if (babyModeState.enabled) {
          ...
          statusElement.textContent = "Baby mode off. Tap a letter card to hear it.";
        }
      });
    ...
    function start() {
      ...
      if (trackElement && statusElement && trackElement.children.length === 0) {
        const babyModeState = setupBabyMode(trackElement, statusElement);
        buildCards(trackElement, statusElement, babyModeState);
